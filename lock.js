...
window.GIMLock = (function () {
  ...
  var build = '';          // 빌드 표식 — 도판 주소에 붙여 캐시를 무효화한다.
                           // 없으면 다시 빌드했을 때 옛 enc 파일이 캐시에서 나와 복호화가 실패한다.
  ...
  function img(el) {
    ...
    fetch('enc/' + encodeURIComponent(name) + '.enc' + (build ? '?b=' + build : ''),
          { cache: 'force-cache' })
      ...
      .catch(function () {
        /* 조용히 빈 칸으로 두면 수업 중에 무슨 일인지 알 수 없다 */
        el.replaceWith(Object.assign(document.createElement('div'), {
          className: 'figfail', textContent: '도판을 열지 못했습니다 (' + name + ')'
        }));
      });
  }
  ...
  var HTML = '' +
    ...
    '<div class="ico">🔒</div>' +
    '<h1>공업 일반 마스터</h1>' +
    '<p>수업용 자료입니다.<br>선생님께 받은 이번 주 코드를 넣어 주세요.</p>' +
  ...
  return { mount: mount, open: open, img: img, fill: fill, forget: forget, codes: codes, panel: panel,
